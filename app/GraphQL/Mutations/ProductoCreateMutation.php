...
class ProductoCreateMutation extends Mutation
{
    ...
    public function resolve($root, $args) // , $context, ResolveInfo $resolveInfo, Closure $getSelectedFields)
    {
        ...
        $prod->user_create = auth()->user()->username;;
    
        // Save the model
        $prod->save();

        // return $prod;
        return Producto::where('id', '=', Producto::max('id'))->firstOrFail();
    }
}